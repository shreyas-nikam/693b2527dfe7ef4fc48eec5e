...
if st.session_state.current_page == "Dashboard":
    ...
    # Using on_change for selectbox to update session state immediately
    ...
    plt.close(fig_heatmap) # Close the figure to prevent display issues in Streamlit
    ...
    # FIX: Changed to r-string and single backslash for LaTeX commands
    ...
    # Safely get default index for selected_employee_id_whatif
    default_employee_index_whatif = 0
    if st.session_state.selected_employee_id_whatif in employee_ids:
        default_employee_index_whatif = employee_ids.index(st.session_state.selected_employee_id_whatif)
    ...
    # Safely get default index for selected_pathway_id_whatif
    default_pathway_index_whatif = 0
    if st.session_state.selected_pathway_id_whatif != "N/A" and not st.session_state.df_pathways.empty:
        default_pathway_name_whatif_list = st.session_state.df_pathways[st.session_state.df_pathways['pathway_id'] == st.session_state.selected_pathway_id_whatif]['pathway_name'].tolist()
        if default_pathway_name_whatif_list and default_pathway_name_whatif_list[0] in pathway_names:
            default_pathway_index_whatif = pathway_names.index(default_pathway_name_whatif_list[0])
    elif not pathway_names: # If no pathways available, set to "N/A" and disable selectbox implicitly
        selected_pathway_name = "N/A"
    else:
        selected_pathway_name = pathway_names[default_pathway_index_whatif]
    ...
    if pathway_names:
        ...
        selected_pathway_id = "N/A" # No pathways to select
    ...
    if st.button("Simulate Pathway Impact"):
        if st.session_state.selected_employee_id_whatif != "N/A" and st.session_state.selected_pathway_id_whatif != "N/A":
            current_employee = st.session_state.df_employees[st.session_state.df_employees['employee_id'] == st.session_state.selected_employee_id_whatif]
            if not current_employee.empty:
                current_ai_r_val = current_employee['current_ai_r_score'].iloc[0]
                ...
                st.warning("Selected employee not found in data.")
        else:
    ...
    if st.session_state.whatif_results and st.session_state.whatif_results["selected_employee_id"] == st.session_state.selected_employee_id_whatif:
        ...
        # FIX: Changed to rf-string and single backslash for \Delta for MathJax within f-string
        ...
        # Logic for plot_current_vs_projected_ai_r
        current_ai_r_val = res['current_ai_r']
        projected_ai_r_vals = [res['projected_ai_r']]
        labels = [res['pathway_name']]

        labels_full = ['Current AI-R'] + [f'Projected AI-R ({label})' for label in labels]
        data = [current_ai_r_val] + projected_ai_r_vals

        bars = ax_whatif.bar(labels_full, data, color=['skyblue'] + ['lightcoral'] * len(projected_ai_r_vals))
        ax_whatif.set_ylabel('AI-R Score')
        ax_whatif.set_title(f'Current vs. Projected AI-R Score for Employee {res["selected_employee_id"]}')
        ax_whatif.set_ylim(0, 100)
        
        # Add values on top of bars
        for bar in bars:
            yval = bar.get_height()
            ax_whatif.text(bar.get_x() + bar.get_width()/2, yval + 1, round(yval, 2), ha='center', va='bottom')

        st.pyplot(fig_whatif)
        plt.close(fig_whatif) # Close the figure to prevent display issues in Streamlit
    ...
    # FIX: Changed to r-string and single backslash for \lambda and \sum
    ...
    # Safely get default index for selected_employee_id_opt
    default_employee_index_opt = 0
    if st.session_state.selected_employee_id_opt in employee_ids_opt:
        default_employee_index_opt = employee_ids_opt.index(st.session_state.selected_employee_id_opt)
    ...
    # FIX: Changed to r-string and single backslash for \lambda for slider label
    ...
    if st.button("Optimize Pathways"):
        if st.session_state.selected_employee_id_opt != "N/A":
            current_employee_opt = st.session_state.df_employees[st.session_state.df_employees['employee_id'] == st.session_state.selected_employee_id_opt]
            if not current_employee_opt.empty:
                current_ai_r_opt_val = current_employee_opt['current_ai_r_score'].iloc[0]
    ...
    if st.session_state.optimization_results and st.session_state.optimization_results["selected_employee_id"] == st.session_state.selected_employee_id_opt:
        ...
        # Logic for plot_current_vs_projected_ai_r
        current_ai_r_val = opt_res['current_ai_r']
        projected_ai_r_vals = [opt_res['projected_final_ai_r']]
        labels = ['Optimized Pathway Sequence']

        labels_full = ['Current AI-R'] + [f'Projected AI-R ({label})' for label in labels]
        data = [current_ai_r_val] + projected_ai_r_vals

        bars = ax_opt.bar(labels_full, data, color=['skyblue'] + ['lightcoral'] * len(projected_ai_r_vals))
        ax_opt.set_ylabel('AI-R Score')
        ax_opt.set_title(f'Current vs. Projected AI-R Score for Employee {opt_res["selected_employee_id"]}')
        ax_opt.set_ylim(0, 100)

        # Add values on top of bars
        for bar in bars:
            yval = bar.get_height()
            ax_opt.text(bar.get_x() + bar.get_width()/2, yval + 1, round(yval, 2), ha='center', va='bottom')

        st.pyplot(fig_opt)
        plt.close(fig_opt) # Close the figure to prevent display issues in Streamlit
    ...
    # FIX: Changed to r-string and single backslash for \alpha, \beta, \%
    st.markdown(r"$$AI-R_{i,t} = \alpha \cdot V^R_{i}(t) + (1 - \alpha) \cdot H^R(t) + \beta \cdot Synergy\%(V^R, H^R)$$")
    st.markdown(f"where:")
    # FIX: Changed to r-string and single backslash for \alpha
    ...
    # FIX: Changed to r-string and single backslash for \beta
    ...
    # FIX: Changed to r-string and single backslash for \%
    ...
    # FIX: Changed `f""` to `rf""` to correctly handle `\text` within an f-string
    st.markdown(rf"The Base Opportunity Score ($H_{\text{base}}(o)$) aggregates the various dimensions of occupational attractiveness: AI-Enhancement Potential, Job Growth Projections, Wage Premium, and Entry Accessibility. It is calculated as a weighted sum:")
    ...
    # FIX: Changed to r-string and single backslash for LaTeX commands
    st.markdown(r"$$S_{i,3} = \frac{{\text{Errors Caught}}}_i}{{\text{Total AI Errors}}} + \frac{{\text{Appropriate Trust Decisions}}}_{i}}{{\text{Total Decisions}}}$$")
    ...
    # FIX: Changed to r-string and single backslash for LaTeX commands
    st.markdown(r"$$S_{i,4} = \frac{{\Delta \text{Proficiency}}}_{i}}{{\Delta t}} \cdot \frac{1}{{\text{Hours Invested}}}$$")
    ...
    # FIX: Changed to r-string and single backslash for LaTeX commands
    st.markdown(r"$$Alignment_i = \frac{{\text{Skills Match Score}}}_{i}}{{\text{Maximum Possible Match}}} \times {\text{Timing Factor}}_{i}$$")
