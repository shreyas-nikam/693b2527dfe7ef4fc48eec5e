...
st.markdown(r"""
In this lab, the **"Workforce AI-Readiness & Upskilling Strategizer"** application is designed to empower AI Workforce leaders and HR executives with a data-driven tool to assess, simulate, and optimize their workforce's preparedness for AI-enabled careers. It implements the **AI-Readiness Score (AI-R)** framework, quantifying individual capabilities and market opportunities to guide strategic talent development and upskilling initiatives.

**Story Flow:**
1.  **Framework Introduction**: Begin by exploring the core AI-Readiness Score (AI-R) framework, its components, and underlying formulas. This is crucial for understanding the metrics used throughout the app.
2.  **Workforce Dashboard**: Get an aggregated overview of the workforce's current AI-R scores and broad skill gaps, broken down by departments or job roles. This provides a quick assessment of the current talent pool's state.
3.  **What-If Scenario Engine**: Select an individual employee and a specific learning pathway to simulate the immediate impact of completing that pathway on their AI-R score. This helps in evaluating program effectiveness at an individual level.
4.  **Multi-Step Pathway Optimization**: For more complex transitions, identify an optimal sequence of learning pathways for a selected employee, balancing AI-R improvement with time and cost constraints, offering a strategic roadmap.
5.  **Strategic Recommendations**: Synthesize insights from the analysis and simulations to generate actionable recommendations for targeted upskilling initiatives and overall workforce development plans.

Use the sidebar navigation to explore each section and gain comprehensive insights into your workforce's AI readiness.
""")
...
with st.sidebar:
    ...
    if st.button("Systematic Opportunity ($H^R$)", key="nav_hrr"):
        st.session_state.current_page = "Systematic Opportunity ($H^R$)"
...
if st.session_state.current_page == "Dashboard":
    ...
    current_group_by_index = 0 if st.session_state.report_group_by == 'job_role' else 1
    st.session_state.report_group_by = st.selectbox(
        "Group AI-Readiness Report by:",
        ['job_role', 'department'],
        index=current_group_by_index,
    ...
    default_employee_index_whatif = employee_ids.index(st.session_state.selected_employee_id_whatif) if st.session_state.selected_employee_id_whatif in employee_ids else 0
    ...
    default_pathway_name_whatif = pathway_names[0] # Fallback
    if st.session_state.selected_pathway_id_whatif != "N/A" and st.session_state.selected_pathway_id_whatif in pathway_map.values():
    ...
    default_pathway_index_whatif = pathway_names.index(default_pathway_name_whatif) if default_pathway_name_whatif in pathway_names else 0
    ...
    if st.button("Simulate Pathway Impact"):
        if st.session_state.selected_employee_id_whatif != "N/A" and st.session_state.selected_pathway_id_whatif != "N/A":
            current_employee_df = st.session_state.df_employees[st.session_state.df_employees['employee_id'] == st.session_state.selected_employee_id_whatif]
            if not current_employee_df.empty:
                current_ai_r_val = current_employee_df['current_ai_r_score'].iloc[0]
                ...
                st.warning(f"Employee ID {st.session_state.selected_employee_id_whatif} not found.")
        else:
    ...
    if st.session_state.whatif_results and st.session_state.whatif_results["selected_employee_id"] == st.session_state.selected_employee_id_whatif:
        ...
        st.markdown(r"Change in AI-R ($\Delta$AI-R): **" f"{res['delta_ai_r']:.2f}**")

        fig_whatif, ax_whatif = plt.subplots(figsize=(8, 5))
        plot_current_vs_projected_ai_r(res['current_ai_r'], [res['projected_ai_r']], [res['pathway_name']], fig=fig_whatif, ax=ax_whatif)
        st.pyplot(fig_whatif)
        plt.close(fig_whatif) # Close figure to prevent displaying multiple times
    ...
    default_employee_index_opt = employee_ids_opt.index(st.session_state.selected_employee_id_opt) if st.session_state.selected_employee_id_opt in employee_ids_opt else 0
    ...
    if st.button("Optimize Pathways"):
        if st.session_state.selected_employee_id_opt != "N/A":
            current_employee_df_opt = st.session_state.df_employees[st.session_state.df_employees['employee_id'] == st.session_state.selected_employee_id_opt]
            if not current_employee_df_opt.empty:
                current_ai_r_opt_val = current_employee_df_opt['current_ai_r_score'].iloc[0]
                ...
                st.warning(f"Employee ID {st.session_state.selected_employee_id_opt} not found.")
        else:
    ...
    if st.session_state.optimization_results and st.session_state.optimization_results["selected_employee_id"] == st.session_state.selected_employee_id_opt:
        ...
        plot_current_vs_projected_ai_r(
            ...
            fig=fig_opt, ax=ax_opt
        )
        st.pyplot(fig_opt)
        plt.close(fig_opt) # Close figure to prevent displaying multiple times
    ...
    st.markdown(f"Based on the AI-Readiness assessment, skills gap analysis, and 'What-If' scenario simulations, we can formulate strategic recommendations for workforce development. These insights move beyond static skill inventories, providing a dynamic framework for continuous adaptation in an an AI-transformed landscape.")
    ...
    st.markdown(r"Identify employees with significantly lower AI-R scores and analyze whether their low score is primarily driven by low Idiosyncratic Readiness ($V^R$) or insufficient Systematic Opportunity ($H^R$) in their current role. Tailor interventions accordingly.")
    ...
    st.markdown(r"*   **Action:** For employees with low $V^R$, recommend AI-Fluency focused training. For those with low $H^R$, consider internal mobility options or upskilling for roles with higher market opportunity.")
    ...
    st.markdown(r"Based on the Skills Gap Heatmap (from the Dashboard), common weaknesses can be identified. For instance, if 'Business Analyst' roles show lower 'Adaptive-Capacity' scores, a targeted training program focusing on 'Strategic Career Management' or 'Cognitive Flexibility' would be beneficial.")
    st.markdown(r"*   **Example:** If 'Data Scientist' roles generally excel in 'AI-Fluency' but show gaps in 'Domain-Expertise', prioritize advanced domain-specific AI applications and certifications.")
    ...
    st.markdown(r"*   **Action:** Leverage such optimized pathways to guide individual career development plans, balancing cost, time, and impact.")
    ...
    if not high_hr_low_vr_roles.empty:
        st.markdown(r"Identify job roles that have high Systematic Opportunity ($H^R$) but where the current workforce has lower Idiosyncratic Readiness ($V^R$). These are prime candidates for strategic investment.")
        ...
        st.markdown(r"*   **Action:** For these roles, focused upskilling on AI-Fluency and Domain-Expertise (specific to the high $H^R$ area) will yield high returns.")
    else:
        st.markdown(r"All employees in this simulation appear to have a balanced $H^R$ and $V^R$, or no explicit high opportunity/low readiness roles were identified.")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"This section demonstrates how the final AI-R score is computed by combining the Idiosyncratic Readiness ($V^R$), Systematic Opportunity ($H^R$), and Synergy components, weighted by the parameters $\alpha$ and $\beta$. The example reflects a scenario where an individual has strong readiness in a high-opportunity field with good alignment, resulting in a high AI-R score.")

elif st.session_state.current_page == "Systematic Opportunity ($H^R$)":
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"The Base Opportunity Score ($H_{\text{base}}(o)$) aggregates the various dimensions of occupational attractiveness: AI-Enhancement Potential, Job Growth Projections, Wage Premium, and Entry Accessibility. It is calculated as a weighted sum:")
    ...
    st.markdown(r"**1. AI-Enhancement Potential**")
    ...
    st.markdown(f"For simplicity in our synthetic data, we directly include an aggregated `ai_enhancement_potential` for each occupation.")
    
    st.markdown(r"**2. Job Growth Projections**")
    ...
    st.markdown(r"This raw growth rate is then normalized to a scale of $[0, 100]$ using an affine transformation:")
    ...
    st.markdown(r"**3. Wage Premium & Entry Accessibility**")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**1. Growth Multiplier ($M_{\text{growth}}(t)$):** Captures market momentum based on recent changes in job postings.")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**2. Regional Multiplier ($M_{\text{regional}}(t)$):** Adjusts for local labor market conditions and remote work suitability.")
    ...
    st.markdown(r"**1. Technical AI Skills** ($S_{i,1}$, $\theta_1 = 0.30$): Based on Prompting, Tools, Understanding, and Data Literacy scores.")
    st.markdown(r"$$S_{i,1} = \frac{1}{4} (\text{Prompting}_i + \text{Tools}_i + \text{Understanding}_i + \text{DataLit}_i)$$")
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**2. AI-Augmented Productivity** ($S_{i,2}$, $\theta_2 = 0.35$): Measures productivity gains with AI assistance.")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**3. Critical AI Judgment** ($S_{i,3}$, $\theta_3 = 0.20$): Assesses error detection and appropriate trust decisions with AI outputs.")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**4. AI Learning Velocity** ($S_{i,4}$, $\theta_4 = 0.15$): Measures improvement rate per unit time investment.")
    st.markdown(r"$$S_{i,4} = \frac{\Delta \text{Proficiency}_i}{\Delta t} \cdot \frac{1}{\text{Hours Invested}}$$")
    st.markdown(r"For simplicity in this notebook, $\Delta Proficiency_i / \Delta t$ can be approximated as a 'learning_rate' for simulation purposes and then scaled with `hours_invested`.")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**1. Educational Foundation ($E_{\text{education}}$):** Discrete values based on education level (e.g., PhD=1.0, Master's=0.85, Bachelor's=0.70, Associate's/Certificate=0.60, HS+Coursework=0.50).")
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**2. Practical Experience ($E_{\text{experience}}$):** Measured by years of experience with diminishing returns:")
    st.markdown(r"$$E_{\text{experience}} = 1 - e^{-\gamma_{\text{exp}} \cdot Years}$$")
    st.markdown(r"where $\gamma_{\text{exp}} = 0.15$.")
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"**3. Specialization Depth ($E_{\text{specialization}}$):** Reflects specific achievements and recognition in their field:")
    ...
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"*   **Cognitive Flexibility ($C_{\text{cognitive}}$):** Problem-solving in novel contexts, transfer learning, creative application of AI tools.")
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"*   **Social-Emotional Intelligence ($C_{\text{social}}$):** Empathy, negotiation, leadership, human-AI collaboration.")
    # FIX: Changed f-string to raw string literal for LaTeX backslashes
    st.markdown(r"*   **Strategic Career Management ($C_{\text{strategic}}$):** Awareness of AI trends, proactive skill development, network building.")
    ...
    st.markdown(r"where $y$ is years of experience. Note that all timing values are $\leq 1.0$ to ensure Alignment remains bounded at $[0, 1]$.")
