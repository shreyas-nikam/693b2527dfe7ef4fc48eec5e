...
with tab1:
    ...
    st.markdown(f"where:")
    st.markdown(r"$$V_i^R(t)$$ is Idiosyncratic Readiness (individual capability), normalized to $$[0, 100]$$.");
    st.markdown(r"$$H_i^R(t)$$ is Systematic Opportunity (market demand) for the target occupation, normalized to $$[0, 100]$$.");
    st.markdown(r"$$\alpha \in [0,1]$$ is the weight on individual vs. market factors. Default $$\alpha = {ALPHA}$$.");
    st.markdown(r"$$\beta > 0$$ is the synergy coefficient, capturing multiplicative benefits. Default $$\beta = {BETA}$$.");
    st.markdown(r"$$Synergy\% \in [0,100]$$ is in percentage units.");
...
with tab2:
    ...
    with col1:
        ...
        current_edu = st.session_state['alice_profile']['education_level']
        # Safety check for index
        try:
            edu_index = edu_options.index(current_edu)
        except ValueError:
            edu_index = 0
...
with tab3:
    ...
    if st.session_state['alice_vr_score'] is None or not st.session_state['hr_scores']:
        ...
        st.markdown(r"$$V^R(t) = w_1 · AI‑Fluency_i(t) + w_2 · Domain‑Expertise_i(t) + w_3 · Adaptive‑Capacity_i(t)$$")
        ...
        st.markdown(r"$$H^R(t) = H_{{base}}(O_{{target}}) · M_{{growth}}(t) · M_{{regional}}(t)$$")
        ...
        st.markdown(r"$$AI-R_{{i,t}} = \alpha · V_i^R(t) + (1 - \alpha) · H_{{i}}^R(t) + \beta · Synergy\%(V_i^R, H_{{i}}^R)$$")
...
with tab4:
    ...
    st.markdown(r"$$ \max_{{P_1,...,P_K}} (AI‑R_{{proj}} - AI‑R_{{current}}) - \lambda · \sum_{{k=1}}^K Cost(p_k) $$ ")
    ...
    st.markdown(f"where $AI‑R_{{proj}}$ is the projected AI-R after completing pathways, $AI‑R_{{current}}$ is her initial AI-R, $\lambda$ is `LAMBDA_COST_WEIGHT`, $T_{{max}}$ is `MAX_LEARNING_TIME_HOURS`, and $B_{{max}}$ is `MAX_LEARNING_BUDGET_USD`.")
...
with tab5:
    ...
    if st.session_state['alice_vr_score'] is None:
        ...
        with scenario_col1:
            # Handle default index safely
            current_top_role_val = st.session_state['top_role']['Role'] if st.session_state['top_role'] else all_roles_for_scenario[0]
            try:
                idx = all_roles_for_scenario.index(current_top_role_val)
            except ValueError:
                idx = 0
